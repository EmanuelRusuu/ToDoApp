<template>
  <div class="flex flex-col gap-4 p-4">
    <div class="flex justify-between items-center">
      <h2 class="text-lg font-semibold">Finished Todos</h2>
    ...
    <table class="w-full table-fixed border-collapse border border-gray-400">
      ...
      <tbody>
        <tr v-for="todo in finishedTodos" :key="todo._id" class="border border-gray-400">
          ...
          <td class="p-2 border border-gray-400">{{ todo.isChecked ? 'Finished' : 'Pending' }}</td>
...
async function fetchTodos() {
  try {
    const response = await axios.get('http://localhost:3000/finished')
    finishedTodos.value = response.data
  } catch (error) {
...
async function deleteAllTodos() {
  try {
    await axios.delete('http://localhost:3000/finished')
    finishedTodos.value = []
  } catch (error) {
